# -*- coding: utf-8 -*-
"""
Created on Fri Jun  2 09:44:58 2017
...
def main(c):
    ...
    global lamb;
    if (sigmatot !=0):
        lamb = 2*(m2)**(0.5)/((6)**(0.5)*(1+lambda1)**(0.5)*(1-beta1*(1-c))**(0.5)) #mean-free path
    print("Lamb = ", lamb)    
    #Source (discrete)
    ...
    for i in range(1,n+1):
        ...
        while((ilost+iesc+ideath)==0):
            randnum=randomGen();
            if (sigmatot !=0):
                if randnum < lambda1/(1+lambda1):
                    freepath = 0;
                else:
                    xi = (lambda1+1)*(1-randnum)   # adjust random number xi to xi_2 such that: 0 < xi_2 < 1
    ...
    file.write("\n")
    file.write("\n")
    s1 = 2*(m2)**(0.5)/((6)**(0.5)*(1+lambda1)**(0.5)*(1-beta1*(1-c))**(0.5));
    s2 = m2/(1-beta1(1-c))
    s3 = 4*(1+lambda1)**(0.5)*(m2)**(1.5)/((6)**(0.5)*(1+*(1-beta1*(1-c))**(1.5));
    s4 = 10*(1+lambda1)*(m2)**(2)/(3*(1+lambda1)**(0.5)*(1-beta1*(1-c))**(2));
    s5 = 10*(6)**(0.5)*(1+lambda1)**(1.5)*(m2)**(2.5)/(3*(1-beta1*(1-c))**(2.5));
    s6 = 70*(1+lambda1)**(2)*(m2)**(3)/(3*(1-beta1*(1-c))**(3));
    file.write("   ---------------------------------------------------------------------------------------\n")
    file.write("   |                  |                    |                      |                      |\n")
