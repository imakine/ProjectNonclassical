...
def main():
    ...
    fluxTR = open("fluxTR.txt","w")
    varianceTR = open("varianceTR.txt","w");
    deviationTR = open("deviationTR.txt","w");                
    coordTR = open("coordTR.txt","w")
    ...
    flux_track = flux_track/numscattot;    
    flux_col = flux_col/numscattot;
    flux_abs = flux_abs/numdeath;
    for i in range(len(fluxpic)):
    ...
    if (numscattot !=0):
        s[:] = [x / (numscattot+numdeath) for x in s];
    finaltime = time.time();
    lon = thickness//2;
    ...
    file.write("   |         Transport EQUATION (1D - Continuous source)      |\n")
    ...
    file.close();    
    for i in range(len(flux_local)):
        fluxTR.write("   "+ str(flux_local[i])+"\n");
    for i in range(len(flux_local)):
        coordTR.write("   "+ str(z)+"\n");
        z = z + step
    for i in range(len(flux_local)):
        varianceTR.write("   "+ str(variancepic[i])+"\n");
    for i in range(len(flux_local)):
        deviationTR.write("   "+ str(variancepic[i])+"\n");
    deviationTR.close()                     
    varianceTR.close()    
    fluxTR.close();
    coordTR.close();          
